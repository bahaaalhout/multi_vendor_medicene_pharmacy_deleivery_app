...
class _MedicationReminderPageState extends State<MedicationReminderPage> {
  ...
  Widget build(BuildContext context) {
    final List<MedicineModel> prescriptionMedicinesLocal = medicineData
        .where((m) => m.requiresPrescription)
        .toList();

    final List<MedicineModel> recentOrderMedicinesLocal = medicineData
        .where((m) => !m.requiresPrescription)
        .toList();

    final items = act.getItemsByTab(
      tab: tab,
      prescriptions: prescriptionMedicinesLocal,
      recents: recentOrderMedicinesLocal,
    );

    final selectedMedicines = act.getSelectedMeds(
      selectedIds: selectedMedicineIds,
      listA: prescriptionMedicinesLocal,
      listB: recentOrderMedicinesLocal,
    );
    ...
    final effectiveSchedule = resolveEffectiveSchedule(
      ...
      allMedicines: [
        ...prescriptionMedicinesLocal,
        ...recentOrderMedicinesLocal,
      ],
      reminders: reminders,
    );
              ...
              child: Column(
                ...
                children: [
                  ...
                  MedicationReminderTabs(
                    selected: tab,
                    prescriptionsCount: prescriptionMedicinesLocal.length,
